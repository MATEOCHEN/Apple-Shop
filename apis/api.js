              ...
              if (err) {
                ...
                for (let i = 0; i < imageList.length; i++) {
                  for (let j = 0; j < result.length; j++) {
                    let productId = result[j].id;
                    if (productId === imageList[i].id) {
                      result[j].img = imageList[i].image;
                    }
                  }
                }
  ...
  mySqlDb.query(
    ...
    (err, result) => {
      if (err) {
        ...
        // 若有相同的個資則將密碼隱藏後回傳至前端, 若找不到則表示該使用者帳號密碼輸入錯誤
...
router.route("/register").post((req, res) => {
  ...
  let data = req.body;
  mySqlDb.query(
    "INSERT INTO custaccount (account, password, type, name, cellphone, email, birthday)" +
      "VALUES (?, ?, ?, ?, ?, ?, ?);",
    [
      data.account,
      data.password,
...
// 新增商品
router.route("/product").post(upload.single("img"), (req, res) => {
  let data = req.body;
  let file = req.file;
  let imgString;
  // 將檔案轉成 base64 字串
